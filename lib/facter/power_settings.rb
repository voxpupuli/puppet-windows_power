...
Facter.add(:power_settings) do
  ...
  setcode do
    ...
    output.each_line('', chomp: true) do |block|
      guid = ''
      ...
      ac = lines[-2].split(': ')[1].nil? ? 0 : lines[-2].split(': ')[1].to_i(0) # AC in seconds
      dc = lines[-1].split(': ')[1].nil? ? 0 : lines[-1].split(': ')[1].to_i(0) # DC in seconds
